        ...
        <div className="flex flex-row gap-12 mb-10 relative">
          <div className="w-1/3 relative top-72 ">
            <div className="flex items-start gap-4 mb-4">
              ...
              <h2 className="text-2xl font-bold text-gray-900">Luxury Residences</h2>
            </div>
            <p className="text-gray-700 leading-relaxed">
              Experience unparalleled elegance in our luxury residences, featuring exquisite design,
              premium amenities, and prime locations for the most discerning tastes.
            </p>
          </div>
          ...
          <div className="relative w-2/3 h-120 rounded-xl overflow-hidden">
            <div className="w-full h-full relative rounded-xl overflow-hidden">
              ...
              <div className="absolute bottom-0 left-0 p-8 text-white">
                <div className="text-7xl font-bold mb-1">{propertyDetails[activeProperty].id}</div>
                <div className="text-xl font-semibold">{propertyDetails[activeProperty].title}</div>
              </div>
            </div>
            ...
            <div className="absolute right-5  top-52 gap-12 h-full flex flex  justify-between">
              {Object.entries(propertyDetails).map(([key, value]) => (
                ...
                >
                  <div
                    className={`absolute right-full top-0 transition-opacity duration-300 ${
                      activeProperty === key ? 'opacity-100' : 'opacity-0'
                    }`}
                  ></div>

                  <div
                    className={`text-6xl font-bold ${activeProperty === key ? 'text-gray-900' : 'text-gray-300'}`}
                  >
                    {value.id}
                  </div>
                  {value.title.split(' ').map((word, idx) => (
                    <div
                      key={idx}
                      className={`writing-vertical transform rotate-180 text-xs tracking-widest mt-1 ${
                        activeProperty === key ? 'text-gray-900 font-medium' : 'text-gray-500'
                      }`}
                    >
                      {word.toUpperCase()}
                    </div>
